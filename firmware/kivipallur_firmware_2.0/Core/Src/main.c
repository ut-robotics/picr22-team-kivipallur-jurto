...
typedef enum {
  MCU_OK,                 ///< OK
  MCU_USB_BAD_DATA,       ///< Got bad data from USB
  MCU_SPEED_TOO_HIGH      ///< Got too high speed from master
} mcu_errors_t;


typedef struct {
  int16_t speeds[3];      ///< Incoming speed array
  uint16_t thrower_speed; ///< Incoming thrower speed
  /*
...
typedef struct {
  mcu_errors_t error;     ///< Error has occured
...
void wake_drivers_up() {
    HAL_GPIO_WritePin(GPIOB, M_sleep_Pin, GPIO_PIN_RESET);
    for(uint16_t i = 0; i < 350; i++) __asm("nop");
	HAL_GPIO_WritePin(GPIOB, M_sleep_Pin, GPIO_PIN_SET);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  ...
  HAL_GPIO_TogglePin(GPIOF, LED_B1_Pin);
...
int main(void)
{
  ...
  while (1)
  {
	  if(is_command_received) {
	  	  ...
	  	  if(!mcu_error) {
	  		HAL_GPIO_TogglePin(GPIOF, LED_R2_Pin);
	  		update_motor_status();
	  	  }
	  	  feedback_needed = 1;
	  	}
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOF, LED_B1_Pin|LED_R2_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(LED_B1_GPIO_Port, LED_B1_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, M_sleep_Pin|M_off_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : LED_B1_Pin LED_B2_Pin */
  GPIO_InitStruct.Pin = LED_B1_Pin|LED_R2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOF, &GPIO_InitStruct);

  /*Configure GPIO pin : LED_B1_Pin */
  GPIO_InitStruct.Pin = LED_B1_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(LED_B1_GPIO_Port, &GPIO_InitStruct);

  /*Configure GPIO pin : BALL_FINDER_Pin */
  GPIO_InitStruct.Pin = LED_B2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(LED_B2_GPIO_Port, &GPIO_InitStruct);

  /*Configure GPIO pins : M_sleep_Pin M_off_Pin */
  GPIO_InitStruct.Pin = M_sleep_Pin|M_off_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
